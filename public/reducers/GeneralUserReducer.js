...
import {
    GENERALUSER_LIST_START, GENERALUSER_LIST_END,GENNERALUSER_DETAIL_START,GENNERALUSER_DETAIL_END,SUBTYPE_LIST_START,SUBTYPE_LIST_END
...
export function getSubtypeList(state = {data: "", fetching: false}, action) {
    switch (action.type) {
        case SUBTYPE_LIST_START:
            state = {...state, fetching: true};
            return state;
        case SUBTYPE_LIST_END:
            state = {data: action.json, fetching: false};
            return state;
        default:
            return state;
